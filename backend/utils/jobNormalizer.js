...
function compressJD(text, { maxBullets = 12 } = {}) {
  ...
  if (bullets.length === 0) {
    ...
    for (const raw of sentences) {
      ...
      if (kept.length >= 5 && kept.length <= 30) bullets.push(kept.join(' '));
...
function normalize(row, { fetchedAt = null } = {}) {
  ...
  return {
    ...
    job: {
      ...
      work_mode,
      employment_type,
      apply_type: row.apply_type ? String(row.apply_type).trim() : null,
      apply_destination: row.apply_link ? String(row.apply_link).trim() : null,
      ...
      response_signal: meta.response_signal,
